...
# değer aralıkları tanımlanıyor
x_tempTest = np.arange(0, 101, 1)
x_firmTest = np.arange(0, 101, 1)
y_quality = np.arange(0, 101, 1)

# belirlenen dilsel değerlerin her biri için üyelik fonksiyonları oluşturuluyor
# trimf metodu üçgen üyelik fonksiyonu
# trapmf metodu yamuk üyelik fonksiyonu tanımlar
...
# subplots metodu aynı anda birden fazla grafiğin çizilmesini sağlar
...
# grafiğin kenarında renklerin ifade ettiklerini belirtir
ax0.legend()
...
# tight_layout, subplot(s) şekil alanına sığacak şekilde alt çizim parametrelerini otomatik olarak ayarlar.
plt.tight_layout()
plt.show()

while True:

    secim = int(input("Veri Seçin : "))

    input_temp, input_firm, degerSonuc = veriler[secim].strip().split(",")

    print("Veri : [Sıcaklık Dayanıklılık : " + input_temp + "\tSağlamlık : " + input_firm + "]")

    # input_temp = int(input("Sıcaklığa Dayanma Değerini giriniz: [ÇIKIŞ (-1)] : "))
    # if (input_temp == -1): exit()
    ...
    # fmax metodu veya; fmin metodu ve anlamındadır (mantıksal bağlaçlar)
    ...
    # birbiri ile aynı çıkışı veren kurallar birleştiriliyor
